...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Roi BB @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n Leia as regras direitinho, se nao o GOXTOSU DO GUMBALL vai te dar ban`
				...
				teks = `Notei que saiu porque nao e digno@${num.split('@')[0]} fraco nao guenta 10 minuto de x1👺🔪`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Tô fazendo krlh, espera aee... ⌛',
				success: '✔️ Viu? Nem demorou deixa de drama✔️',
				error: {
					stick: '⚠️ Deu merda aqui man, ocorreu um erro ao converter a imagem em figurinha ⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
			...
			const ownerNumber = ["558897425534@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Adm deixou? Seu membro comum punhetero😈💦🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Gumball OF SadNess\n*YOUTUBE:* https://youtube.com/channel/UC8BcEDYzzfJ7ypM2-Xqry4Q\n*WPP:* wa.me/+5588997425534\n*INSTA:* https://www.instagram.com/marcostulio_ofc/\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia meus membros, só passando pra avisar que o Gumball ama vcs ❤💖😌})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde, seus baiano💤 kkkk'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite a mimir que amanha tem mais ❤️'})
					break
				case 'belle':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do adm pcw gumball:*\n\n https://youtube.com/channel/UC8BcEDYzzfJ7ypM2-Xqry4Q'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'nsfwloli':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'nem divide rumm😡😡'})
					break
				case 'reislin':
					memein = await kagApi.memeindo()
				...
				case 'ts':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ ISSO E UMA TM FODASE ]\n\n${body.slice(4)}`})
						}
						reply('Transmissão enviada com sucesso')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ ISSO E UMA TM FODASE ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada com sucesso')
					}
					break
        case 'promote':
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKK Random nao obedeceu o Gumball e foi de ban, trouxaaa : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
